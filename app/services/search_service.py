...
def _fetch_video_map(conn, video_ids: list) -> dict:
    ...
    rows = conn.execute(
        f"SELECT id, filename, duration_sec FROM videos WHERE id IN ({placeholders})",
        video_ids,
    ).fetchall()
...
def _fetch_photo_map(conn, photo_ids: list) -> dict:
    ...
    rows = conn.execute(
        f"SELECT id, filename, taken_at FROM photos WHERE id IN ({placeholders})",
        photo_ids,
    ).fetchall()
...
def search_person_by_name(name: str = "", limit: int = 200) -> dict:
    name = name.strip()
    if not name:
        return {"videos": [], "photos": []}
    ...
    return {"videos": video_hits[:limit], "photos": photo_hits[:limit]}


def search_by_reference_image_bytes(data: bytes, distance_threshold: float = 0.5, limit: int = 100) -> dict:
    ...
    video_metas_dist = [
        ...
        if d <= distance_threshold and m.get("media_type") != "photo"
    ]
    photo_metas_dist = [
        (m, d) for m, d in zip(metadatas, distances)
        if d <= distance_threshold and m.get("media_type") == "photo"
    ...
    all_ids = list({m.get("person_id", "unlabeled") for m, d in zip(metadatas, distances) if d <= distance_threshold})
